"""
File Name: porkwindow.py
Name: Caleb Steelman
DLM: May 12th, 2024
Defines and creates a class called PorkWindow, which is used to open a window to calculate the calories, protein, and fat of pork based on poundage.
"""
...
class PorkWindow:
    ...
    def __init__(self, master, main_window):
        ...
        self.top.title("Pork Window")
        self.top.geometry("300x200")
        #The window is made unreziable, so the user must use it's intended dimensions.
        self.top.resizable(width=False, height=False)
        #the image "pig.png" is opened, resized, and then antialised using LANCZOS, set as image.
        image = Image.open("pig.png")
        image = image.resize((300, 200), Image.Resampling.LANCZOS)
        #pig.png is set as photo via image.
        self.photo = ImageTk.PhotoImage(image)
        #Try and except are used to set up alt-text, with it backing up text if it does not work.
        try:
            #The backround is set to photo, which is pig.png, and then modified slightly. This all makes it the background image.
            self.background_label = tk.Label(self.top, image=self.photo)
            self.background_label.place(x=0, y=0, relwidth=1, relheight=1)
        except Exception as e:
            self.background_label = tk.Label(self.top, text="Picture of a Pig") #Alternate Text
        ...
        self.button = tk.Button(self.top, text="Calculate", command=self.calculatee)
